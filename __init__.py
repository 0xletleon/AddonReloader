...
class ADDONRELOADER_OT_reload_addon(bpy.types.Operator):
    """定义重新加载插件的操作类 Define the operator class for reloading addons"""
    bl_idname = "addonreloader.reload_addon"
    bl_label = "Reload Addon"
    bl_description = "Disable and re-enable the selected addon"

    def execute(self, context):
        """执行重新加载插件的操作 Execute the addon reloading operation"""
...
class ADDONRELOADER_OT_refresh_list(bpy.types.Operator):
    """定义刷新插件列表的操作类 Define the operator class for refreshing the addon list"""
    bl_idname = "addonreloader.refresh_list"
    bl_label = "Refresh Addon List"
    bl_description = "Refresh the list of user-installed addons"

    def execute(self, context):
        """执行刷新插件列表的操作 Execute the addon list refreshing operation"""
        update_addon_list()
        self.report({'INFO'}, "Addon list refreshed")
        return {'FINISHED'}

def get_addon_list(self, context):
    """返回插件列表，用于EnumProperty Return the addon list for EnumProperty"""
    global addon_list
    return addon_list

def is_user_addon(module_name):
    """检查插件是否为用户安装的插件 Check if the addon is user-installed"""
...
def update_addon_list():
    """更新可重新加载的插件列表 Update the list of reloadable addons"""
    global addon_list
...
def register():
    """注册插件的类和属性 Register the addon's classes and properties"""
    update_addon_list()
